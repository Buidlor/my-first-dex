...
it("should throw an error if ETH balance is too low ceating BUY limit order", async () => {
 ...
 it("should throw an eror if the token balance is too low when creating SELL limit order", async() =>{
     let dex = await Dex.deployed()
     let link = await Link.deployed()
     await truffleAssert.reverts(
    ...
    it("The SELL order book should be ordered on price from lowest to highest starting at index 0", async () => {
        ...
        assert(orderbook.length > 0, "no oders in the sell book");
